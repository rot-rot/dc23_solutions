words = """aardvark aardwolf aasvogel abacuses abalones abampere abandons abapical
...
zymogene zymogens zymogram zymology zymosans zyzzyvas""".replace("\n"," ").split(" ")

print len(words)

           #DARTHNULLDARTHNULLDARTHNULL
keyword = "SOOPERSEKRITKEYGOESDOWNHERE"
#keyword =  "ABCDEFGHIJKLMNOPQRSTUVWXYZ."
#keyword = "DARTHNULLDARTHNULLDARTHNULL"
#keyword="DARTHNULLDARTHNULLDARTHNULL"
...
c1 = "184392767555233165749994723531259495865376276238379421181442353253697113145966377188613181592"
c2 = ("1982649565652386773691131973279" +
      "1743436171585426385362139571445" +
      "6782198976785171492852369717546" +
      "7936959996891458126213928623258" +
      "19818724")

KEYLEN = 9
common = ['E','T','A','O','I']
stats = {}
def decode(s,keyword):
  grid = []
  for i in range(27):
    grid.append(alphabet[alphabet.index(keyword[i % len(keyword)])::1 if i % 2 == 0 else -1]
                + alphabet[:alphabet.index(keyword[i % len(keyword)]):1 if i % 2 == 0 else -1])
  #print "\n".join(grid)
  out = ""
  for i in range(0,len(s)/3):
    sq = int(s[i])
    row = int(s[i + len(s)/3])
    col = int(s[i + 2*len(s)/3])
    #sq = int(c1[i*3])
    #row = int(c1[i*3 + 1])
    #col = int(c1[i*3 + 2])
    ...
    col_offset = ((sq-1) % 3) * 9
    row_offset = int((sq-1) / 3) * 9
    y = row - 1 + row_offset
    x = col - 1 + col_offset


    
    #print grid[y][x],sq, row, col, y, x
    out+= grid[y][x]
  return out
...
def crack(s):
  ...
  for i in range(0,len(s)/3):
    ...
    for c in common:
      if row % 2 == 0:
        stats[row % KEYLEN].append(alphabet[(alphabet.index(c) - col)%len(alphabet)])
      else:
        stats[row % KEYLEN].append(alphabet[(alphabet.index(c) + col) % len(alphabet)])
...
def crack2(s,k):
  ...
  ret = []
  for offset in range(0,len(s)/3 - len(k)):
    key = "x" * 27

    valid = True
    for i in range(offset,offset+len(k)):
      sq = int(s[i])
      row = int(s[i + len(s)/3])
      col = int(s[i + 2*len(s)/3])
      ...
      alpha = ""
      if row % 2 == 0:
        alpha = alphabet[(alphabet.index(k[i-offset]) - col)%len(alphabet)] 
      else:
        alpha = alphabet[(alphabet.index(k[i-offset]) + col)%len(alphabet)]

      #print alpha,k[i-offset],row,col

      if key[row] != "x" and key[row] != alpha:
        valid = False
      key = key[:row] + alpha + key[row+1:]
    ret.append((str(valid),key))
...
print decode(c1,"SOOPERSEKRITKEYGOESDOWNHERE")
...
print "Take a guess..."
while 1:
  #r = crack2(c2,raw_input().upper())
  #print "\n".join([str(x) for x in r])
  #print decode(c2,r[0][1].upper())
  print decode(c2,raw_input().upper())
